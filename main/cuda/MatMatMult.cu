...
__global__ void _dev_MatMatMultNew(const TYPE * __restrict__ A,
  ...
  for (int n=0; n<N_UNROLL; ++n) {
    for (int p=0; p<N_UNROLL; ++p) {
      if (r<dev_nRowsA) {
	if (c+p<dev_nColsB)
	  C[r*dev_nColsB + c+p] = Creg[n*N_UNROLL+p];
      }
    }
    r++;
  }
...
kmeansCudaErrorStatus MatMatMult(const int nRowsA, const int nColsA, const TYPE *A, 
  ...
  try {
    ...
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(dev_nRowsAPadded, &nRowsAPadded, sizeof(int)),ERROR_MATMATMULT);
    ...
    if (delta==1) _dev_MatMatMultNew<TYPE,N_UNROLL,1><<<grid,block>>>(A,B,C);
    if (delta==2) _dev_MatMatMultNew<TYPE,N_UNROLL,2><<<grid,block>>>(A,B,C);
    if (delta==3) _dev_MatMatMultNew<TYPE,N_UNROLL,3><<<grid,block>>>(A,B,C);
    if (delta==4) _dev_MatMatMultNew<TYPE,N_UNROLL,4><<<grid,block>>>(A,B,C);
    if (delta==5) _dev_MatMatMultNew<TYPE,N_UNROLL,5><<<grid,block>>>(A,B,C);
    if (delta==6) _dev_MatMatMultNew<TYPE,N_UNROLL,6><<<grid,block>>>(A,B,C);
    if (delta==7) _dev_MatMatMultNew<TYPE,N_UNROLL,7><<<grid,block>>>(A,B,C);
    if (delta==8) _dev_MatMatMultNew<TYPE,N_UNROLL,8><<<grid,block>>>(A,B,C);
    if (delta==9) _dev_MatMatMultNew<TYPE,N_UNROLL,9><<<grid,block>>>(A,B,C);
    if (delta==10) _dev_MatMatMultNew<TYPE,N_UNROLL,10><<<grid,block>>>(A,B,C);
    if (delta==11) _dev_MatMatMultNew<TYPE,N_UNROLL,11><<<grid,block>>>(A,B,C);
    if (delta==12) _dev_MatMatMultNew<TYPE,N_UNROLL,12><<<grid,block>>>(A,B,C);
    if (delta==13) _dev_MatMatMultNew<TYPE,N_UNROLL,13><<<grid,block>>>(A,B,C);
    if (delta==14) _dev_MatMatMultNew<TYPE,N_UNROLL,14><<<grid,block>>>(A,B,C);
    if (delta==15) _dev_MatMatMultNew<TYPE,N_UNROLL,15><<<grid,block>>>(A,B,C);
    if (delta==16) _dev_MatMatMultNew<TYPE,N_UNROLL,16><<<grid,block>>>(A,B,C);
#else
